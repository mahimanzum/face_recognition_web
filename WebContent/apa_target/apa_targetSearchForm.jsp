...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.APA_TARGET_EDIT_SEMENCOLLECTION, loginDTO)%></th>
								<th><%=LM.getText(LC.APA_TARGET_EDIT_ARTIFICIALINSEMENATION, loginDTO)%></th>
								<th><%=LM.getText(LC.APA_TARGET_EDIT_PROGENY, loginDTO)%></th>
								<th><%=LM.getText(LC.APA_TARGET_EDIT_CANDIDATEBULLPRODUCTION, loginDTO)%></th>
								<th><%=LM.getText(LC.APA_TARGET_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%=LM.getText(LC.APA_TARGET_EDIT_ENTRYDATE, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.APA_TARGET_SEARCH_APA_TARGET_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.APA_TARGET_SEARCH_APA_TARGET_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_APA_TARGET);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Apa_targetDTO row = (Apa_targetDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_APA_TARGET;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Apa_targetServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append("ID", value);
	formData.append('semenCollection', document.getElementById('semenCollection_text_' + i).value);
	formData.append('artificialInsemenation', document.getElementById('artificialInsemenation_text_' + i).value);
	formData.append('progeny', document.getElementById('progeny_text_' + i).value);
	formData.append('candidateBullProduction', document.getElementById('candidateBullProduction_text_' + i).value);
	...
	xhttp.open("POST", 'Apa_targetServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
