...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.ARTIFICIAL_INSEMENATION_RECORD_EDIT_CENTRETYPE, loginDTO)%></th>
								...
								<th><%=LM.getText(LC.ARTIFICIAL_INSEMENATION_RECORD_EDIT_ENTRYDATE, loginDTO)%></th>
								<th><%=LM.getText(LC.ARTIFICIAL_INSEMENATION_RECORD_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.ARTIFICIAL_INSEMENATION_RECORD_SEARCH_ARTIFICIAL_INSEMENATION_RECORD_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.ARTIFICIAL_INSEMENATION_RECORD_SEARCH_ARTIFICIAL_INSEMENATION_RECORD_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_ARTIFICIAL_INSEMENATION_RECORD);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Artificial_insemenation_recordDTO row = (Artificial_insemenation_recordDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_ARTIFICIAL_INSEMENATION_RECORD;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Artificial_insemenation_recordServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append('noOfAI', document.getElementById('noOfAI_number_' + i).value);
	...
	xhttp.open("POST", 'Artificial_insemenation_recordServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
