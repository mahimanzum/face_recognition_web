...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.CANDIDATE_BULL_PRODUCTION_EDIT_PRODUCTIONDATE, loginDTO)%></th>
								<th><%=LM.getText(LC.CANDIDATE_BULL_PRODUCTION_EDIT_NUMBEROFCANDIDATEBULLS, loginDTO)%></th>
								<th><%=LM.getText(LC.CANDIDATE_BULL_PRODUCTION_EDIT_SOURCETYPE, loginDTO)%></th>
								<th><%=LM.getText(LC.CANDIDATE_BULL_PRODUCTION_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.CANDIDATE_BULL_PRODUCTION_SEARCH_CANDIDATE_BULL_PRODUCTION_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.CANDIDATE_BULL_PRODUCTION_SEARCH_CANDIDATE_BULL_PRODUCTION_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_CANDIDATE_BULL_PRODUCTION);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Candidate_bull_productionDTO row = (Candidate_bull_productionDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_CANDIDATE_BULL_PRODUCTION;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Candidate_bull_productionServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append("ID", value);
	formData.append('productionDate', document.getElementById('productionDate_date_' + i).value);
	formData.append('numberOfCandidateBulls', document.getElementById('numberOfCandidateBulls_number_' + i).value);
	formData.append('sourceType', document.getElementById('sourceType_select_' + i).value);
	...
	xhttp.open("POST", 'Candidate_bull_productionServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
