...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.PERMISSIBLE_BULLS_IN_CENTRE_EDIT_BULLTYPE, loginDTO)%></th>
								...
								<th><%out.print(LM.getText(LC.PERMISSIBLE_BULLS_IN_CENTRE_SEARCH_PERMISSIBLE_BULLS_IN_CENTRE_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.PERMISSIBLE_BULLS_IN_CENTRE_SEARCH_PERMISSIBLE_BULLS_IN_CENTRE_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_PERMISSIBLE_BULLS_IN_CENTRE);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Permissible_bulls_in_centreDTO row = (Permissible_bulls_in_centreDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_PERMISSIBLE_BULLS_IN_CENTRE;
...
function PreprocessBeforeSubmiting(row, validate)
{
	...
	console.log("found date = " + document.getElementById('dateOfExpiration_date_Date_' + row).value);
	document.getElementById('dateOfExpiration_date_' + row).value = new Date(document.getElementById('dateOfExpiration_date_Date_' + row).value).getTime();
	return true;
}
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Permissible_bulls_in_centreServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append('dateOfExpiration', document.getElementById('dateOfExpiration_date_' + i).value);
	...
	xhttp.open("POST", 'Permissible_bulls_in_centreServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
