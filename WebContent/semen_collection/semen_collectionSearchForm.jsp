...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.SEMEN_COLLECTION_EDIT_BULLTYPE, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_COLLECTION_EDIT_NOOFDOSE, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_COLLECTION_EDIT_VOLUME, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_COLLECTION_EDIT_DENSITY, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_COLLECTION_EDIT_PROGRESSIVEMORTALITY, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_COLLECTION_EDIT_COLORTYPE, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_COLLECTION_EDIT_TRANSACTIONDATE, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_COLLECTION_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.SEMEN_COLLECTION_SEARCH_SEMEN_COLLECTION_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.SEMEN_COLLECTION_SEARCH_SEMEN_COLLECTION_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_SEMEN_COLLECTION);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Semen_collectionDTO row = (Semen_collectionDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_SEMEN_COLLECTION;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Semen_collectionServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append('progressiveMortality', document.getElementById('progressiveMortality_text_' + i).value);
	formData.append('colorType', document.getElementById('colorType_select_' + i).value);
	formData.append('transactionDate', document.getElementById('transactionDate_date_' + i).value);
	...
	xhttp.open("POST", 'Semen_collectionServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
