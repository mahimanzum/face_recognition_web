...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.SEMEN_DISTRIBUTION_EDIT_BULLTYPE, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_DISTRIBUTION_EDIT_NOOFDOSE, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_DISTRIBUTION_EDIT_REQUISITIONID, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_DISTRIBUTION_EDIT_GROUPID, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_DISTRIBUTION_EDIT_TRANSACTIONDATE, loginDTO)%></th>
								<th><%=LM.getText(LC.SEMEN_DISTRIBUTION_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.SEMEN_DISTRIBUTION_SEARCH_SEMEN_DISTRIBUTION_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.SEMEN_DISTRIBUTION_SEARCH_SEMEN_DISTRIBUTION_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_SEMEN_DISTRIBUTION);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Semen_distributionDTO row = (Semen_distributionDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_SEMEN_DISTRIBUTION;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Semen_distributionServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append('requisitionId', document.getElementById('requisitionId_text_' + i).value);
	formData.append('groupId', document.getElementById('groupId_text_' + i).value);
	...
	xhttp.open("POST", 'Semen_distributionServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
