...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.APPROVAL_STATUS_EDIT_NAMEEN, loginDTO)%></th>
								<th><%=LM.getText(LC.APPROVAL_STATUS_EDIT_NAMEBN, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.APPROVAL_STATUS_SEARCH_APPROVAL_STATUS_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.APPROVAL_STATUS_SEARCH_APPROVAL_STATUS_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_APPROVAL_STATUS);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Approval_statusDTO row = (Approval_statusDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_APPROVAL_STATUS;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Approval_statusServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	xhttp.open("POST", 'Approval_statusServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
