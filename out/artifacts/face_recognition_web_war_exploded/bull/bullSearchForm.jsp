...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.BULL_EDIT_NAMEEN, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_NAMEBN, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_DATEOFBIRTH, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_BREEDTYPE, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_STATUSTYPE, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_DAM, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_SIRE, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_MATERNALGRANDDAM, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_MATERNALGRANDSIRE, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_PATERNALGRANDDAM, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_PATERNALGRANDSIRE, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_MILKYIELDOFDAM, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_MILKYIELDOFSIRESDAM, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_PROGENYMILKYIELD, loginDTO)%></th>
								<th><%=LM.getText(LC.BULL_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.BULL_SEARCH_BULL_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.BULL_SEARCH_BULL_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_BULL);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											BullDTO row = (BullDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_BULL;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "BullServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append('statusType', document.getElementById('statusType_select_' + i).value);
	...
	xhttp.open("POST", 'BullServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
