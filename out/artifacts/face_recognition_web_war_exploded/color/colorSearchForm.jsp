...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.COLOR_EDIT_NAMEEN, loginDTO)%></th>
								<th><%=LM.getText(LC.COLOR_EDIT_NAMEBN, loginDTO)%></th>
								<th><%=LM.getText(LC.COLOR_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.COLOR_SEARCH_COLOR_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.COLOR_SEARCH_COLOR_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_COLOR);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											ColorDTO row = (ColorDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_COLOR;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "ColorServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	xhttp.open("POST", 'ColorServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
