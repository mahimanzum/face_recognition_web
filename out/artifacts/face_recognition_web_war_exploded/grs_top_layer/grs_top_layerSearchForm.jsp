...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.GRS_TOP_LAYER_EDIT_NAMEEN, loginDTO)%></th>
								<th><%=LM.getText(LC.GRS_TOP_LAYER_EDIT_NAMEBN, loginDTO)%></th>
								<th><%=LM.getText(LC.GRS_TOP_LAYER_EDIT_LAYERNUMBER, loginDTO)%></th>
								<th><%=LM.getText(LC.GRS_TOP_LAYER_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.GRS_TOP_LAYER_SEARCH_GRS_TOP_LAYER_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.GRS_TOP_LAYER_SEARCH_GRS_TOP_LAYER_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_GRS_TOP_LAYER);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Grs_top_layerDTO row = (Grs_top_layerDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_GRS_TOP_LAYER;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Grs_top_layerServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append('layerNumber', document.getElementById('layerNumber_number_' + i).value);
	...
	xhttp.open("POST", 'Grs_top_layerServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
