...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.SOURCE_EDIT_NAMEEN, loginDTO)%></th>
								<th><%=LM.getText(LC.SOURCE_EDIT_NAMEBN, loginDTO)%></th>
								<th><%=LM.getText(LC.SOURCE_EDIT_DESCRIPTION, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.SOURCE_SEARCH_SOURCE_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.SOURCE_SEARCH_SOURCE_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_SOURCE);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											SourceDTO row = (SourceDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_SOURCE;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "SourceServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	xhttp.open("POST", 'SourceServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
