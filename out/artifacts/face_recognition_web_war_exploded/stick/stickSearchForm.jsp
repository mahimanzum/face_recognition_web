...
String value = "";
String Language = LM.getText(LC.STICK_EDIT_LANGUAGE, loginDTO);
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.STICK_EDIT_NAMEEN, loginDTO)%></th>
								<th><%=LM.getText(LC.STICK_EDIT_NAMEBN, loginDTO)%></th>
								<th><%=LM.getText(LC.STICK_EDIT_SEMENCOLLECTIONID, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.STICK_SEARCH_STICK_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.STICK_SEARCH_STICK_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_STICK);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											StickDTO row = (StickDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_STICK;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "StickServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append('semenCollectionId', document.getElementById('semenCollectionId_text_' + i).value);
	...
	xhttp.open("POST", 'StickServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
