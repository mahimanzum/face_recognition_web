...
%>				
				...
				<div class="table-responsive">
					<table id="tableData" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th><%=LM.getText(LC.UNVALIDATED_WORD_EDIT_WORD, loginDTO)%></th>
								<th><%=LM.getText(LC.UNVALIDATED_WORD_EDIT_VERDICTCOUNT, loginDTO)%></th>
								<th><%=LM.getText(LC.UNVALIDATED_WORD_EDIT_ISCORRECT, loginDTO)%></th>
								<th><%out.print(LM.getText(LC.UNVALIDATED_WORD_SEARCH_UNVALIDATED_WORD_EDIT_BUTTON, loginDTO));%></th>
								<th><input type="submit" class="btn btn-xs btn-danger" value="
								<%out.print(LM.getText(LC.UNVALIDATED_WORD_SEARCH_UNVALIDATED_WORD_DELETE_BUTTON, loginDTO));%>
								" /></th>
						...
						<tbody>
							<%
								ArrayList data = (ArrayList) session.getAttribute(SessionConstants.VIEW_UNVALIDATED_WORD);
								...
								try
								{
									...
									if (data != null) 
									{
										...
										{
											Unvalidated_wordDTO row = (Unvalidated_wordDTO) data.get(i);
...
<%
	String navigator2 = SessionConstants.NAV_UNVALIDATED_WORD;
...
function doEdit(params, i, id, deletedStyle)
{
	...
	xhttp.onreadystatechange = function() 
	{
	  ...
	  xhttp.open("Get", "Unvalidated_wordServlet?actionType=getEditPage" + params, true);
	  xhttp.send();	
}
...
function submitAjax(i, deletedStyle)
{
	...
	formData.append("ID", value);
	formData.append('word', document.getElementById('word_database_' + i).files[0]);
	formData.append('verdictCount', document.getElementById('verdictCount_text_' + i).value);
	formData.append('isCorrect', document.getElementById('isCorrect_checkbox_' + i).value);
	...
	xhttp.open("POST", 'Unvalidated_wordServlet?actionType=edit&inplacesubmit=true&deletedStyle=' + deletedStyle + '&rownum=' + i, true);
	xhttp.send(formData);
}
