...
public class MenuConfigurationServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			{
				if(PermissionRepository.checkPermissionByRoleIDAndMenuID(userDTO.roleID, MenuConstants.MENU_SETTINGS_VIEW)){
...
//		String names[] = request.getParameterValues("name");
		...
		for(int isDeletedMenuID: isDeletedIDList){
			toBeDeletedMenuIDList.addAll(MenuRepository.getSubtreeMenuIDListByMenuID(isDeletedMenuID));
		}
