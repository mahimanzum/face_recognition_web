...
@WebServlet("/Liquid_semen_report_Servlet")
public class Liquid_semen_report_Servlet  extends HttpServlet {
	/**
	 * 
	 */
	...
	String Criteria[][] = 
	{
		...
		{"criteria","","collection_date",">","AND","long","","","0","collectionDate"}		
	};
	
	String Display[][] = 
	{
		{"display","","total_dose","text",""},		
		{"display","","total_volume","text",""},		
	...
	public Liquid_semen_report_Servlet(){
			...
			throws ServletException, IOException 
	{
		...
		sql = "(SELECT IFNULL(SUM(no_of_dose), 0) AS total_dose, IFNULL(SUM(volume), 0) AS total_volume, centre.name_en AS centre_name, centre_type, liquid_semen.collection_distribution_date AS collection_date FROM liquid_semen JOIN centre ON liquid_semen.centre_type = centre.id WHERE liquid_semen.collection_distribution_date >= 0 GROUP BY liquid_semen.centre_type) AS derived";

		Display[0][4] = LM.getText(LC.LIQUID_SEMEN_REPORT_SELECT_TOTALDOSE, loginDTO);
		Display[1][4] = LM.getText(LC.LIQUID_SEMEN_REPORT_SELECT_TOTALVOLUME, loginDTO);
		Display[2][4] = LM.getText(LC.LIQUID_SEMEN_REPORT_SELECT_CENTRENAME, loginDTO);

		
		if(PermissionRepository.checkPermissionByRoleIDAndMenuID(userDTO.roleID, MenuConstants.LIQUID_SEMEN_REPORT_DETAILS))
		{
			
			if(ActionTypeConstant.REPORT_PAGE.equals(actionType)){
				
				request.getRequestDispatcher("/liquid_semen_report/liquid_semen_report.jsp").forward(request, response);
