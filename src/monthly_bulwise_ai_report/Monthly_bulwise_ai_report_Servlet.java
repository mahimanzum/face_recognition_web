package monthly_bulwise_ai_report;
...
@WebServlet("/Monthly_bulwise_ai_report_Servlet")
public class Monthly_bulwise_ai_report_Servlet  extends HttpServlet {
	/**
	 * 
	 */
	...
	public Monthly_bulwise_ai_report_Servlet(){
			...
			throws ServletException, IOException 
	{
		...
		sql = "(SELECT centre.ID AS centre_type,"
				...
				+ " WHERE entry_date >= " + l1stDayOfMonth 
				+ " ORDER BY centre_type) AS derived";

		Display[0][4] = LM.getText(LC.MONTHLY_BULWISE_AI_REPORT_SELECT_CENTRENAME, loginDTO);
		Display[1][4] = LM.getText(LC.MONTHLY_BULWISE_AI_REPORT_SELECT_BULLNAME, loginDTO);
		Display[2][4] = LM.getText(LC.MONTHLY_BULWISE_AI_REPORT_SELECT_NOOFAI, loginDTO);
		Display[3][4] = LM.getText(LC.MONTHLY_BULWISE_AI_REPORT_SELECT_ENTRYDATE, loginDTO);

		
		if(PermissionRepository.checkPermissionByRoleIDAndMenuID(userDTO.roleID, MenuConstants.MONTHLY_BULWISE_AI_REPORT_DETAILS))
		{
			
			if(ActionTypeConstant.REPORT_PAGE.equals(actionType)){
				
				request.getRequestDispatcher("/monthly_bulwise_ai_report/monthly_bulwise_ai_report.jsp").forward(request, response);
