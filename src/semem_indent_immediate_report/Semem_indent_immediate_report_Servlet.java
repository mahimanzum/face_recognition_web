package semem_indent_immediate_report;
...
@WebServlet("/Semem_indent_immediate_report_Servlet")
public class Semem_indent_immediate_report_Servlet  extends HttpServlet {
	/**
	 * 
	 */
	...
	String Criteria[][] = 
	{
		{"criteria","","group_id","=","","String","","","any","groupId"},		
		{"criteria","","centre_type","=","AND","int","","","any","centreType"},		
		{"criteria","","breed_type","=","AND","int","","","any","breedType"},		
		{"criteria","","requisition_date","=","AND","long","","","any","requisitionDate"}		
	};
	
	String Display[][] = 
	{
		{"display","","group_id","text",""},		
		{"display","","centre_name","text",""},		
		{"display","","breed_type","text",""},		
		{"display","","no_of_dose","text",""},		
		{"display","","requisition_date","date",""},		
	...
	public Semem_indent_immediate_report_Servlet(){
			...
			throws ServletException, IOException 
	{
		...
		sql = "(SELECT group_id, centre.name_en AS centre_name, breed.name_en AS breed_type, semen_requisition.centre_type AS centre_type, no_of_dose, requisition_date, semen_requisition.description AS description FROM semen_requisition JOIN centre ON semen_requisition.centre_type = centre.id JOIN breed ON semen_requisition.breed_type = breed.id ORDER BY group_id, centre.name_en) AS derived";

		Display[0][4] = LM.getText(LC.SEMEM_INDENT_IMMEDIATE_REPORT_SELECT_GROUPID, loginDTO);
		Display[1][4] = LM.getText(LC.SEMEM_INDENT_IMMEDIATE_REPORT_SELECT_CENTRENAME, loginDTO);
		Display[2][4] = LM.getText(LC.SEMEM_INDENT_IMMEDIATE_REPORT_SELECT_BREEDTYPE, loginDTO);
		Display[3][4] = LM.getText(LC.SEMEM_INDENT_IMMEDIATE_REPORT_SELECT_NOOFDOSE, loginDTO);
		Display[4][4] = LM.getText(LC.SEMEM_INDENT_IMMEDIATE_REPORT_SELECT_REQUISITIONDATE, loginDTO);
		Display[5][4] = LM.getText(LC.SEMEM_INDENT_IMMEDIATE_REPORT_SELECT_DESCRIPTION, loginDTO);

		
		if(PermissionRepository.checkPermissionByRoleIDAndMenuID(userDTO.roleID, MenuConstants.SEMEM_INDENT_IMMEDIATE_REPORT_DETAILS))
		{
			
			if(ActionTypeConstant.REPORT_PAGE.equals(actionType)){
				
				request.getRequestDispatcher("/semem_indent_immediate_report/semem_indent_immediate_report.jsp").forward(request, response);
