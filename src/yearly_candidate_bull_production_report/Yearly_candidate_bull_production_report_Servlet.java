package yearly_candidate_bull_production_report;
...
@WebServlet("/Yearly_candidate_bull_production_report_Servlet")
public class Yearly_candidate_bull_production_report_Servlet  extends HttpServlet {
	/**
	 * 
	 */
	...
	public Yearly_candidate_bull_production_report_Servlet(){
			...
			throws ServletException, IOException 
	{
		...
		sql = "(SELECT source.name_en AS source_name,"
				...
				+ " FROM candidate_bull_production JOIN source ON candidate_bull_production.source_type = source.id"
				+ " WHERE production_date >= " + l1stDayOfJuly + " GROUP BY source_type)"
				+ " AS derived";

		Display[0][4] = LM.getText(LC.YEARLY_CANDIDATE_BULL_PRODUCTION_REPORT_SELECT_SOURCENAME, loginDTO);
		Display[1][4] = LM.getText(LC.YEARLY_CANDIDATE_BULL_PRODUCTION_REPORT_SELECT_TOTALCANDIDATEBULLS, loginDTO);

		
		if(PermissionRepository.checkPermissionByRoleIDAndMenuID(userDTO.roleID, MenuConstants.YEARLY_CANDIDATE_BULL_PRODUCTION_REPORT_DETAILS))
		{
			
			if(ActionTypeConstant.REPORT_PAGE.equals(actionType)){
				
				request.getRequestDispatcher("/yearly_candidate_bull_production_report/yearly_candidate_bull_production_report.jsp").forward(request, response);
